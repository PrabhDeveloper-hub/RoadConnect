...
export class CameraResizer {
  ...
  private getZoom(): number {
    if (
      this.game.scale.gameSize.width > DESIGN_RES.width * this.dpr &&
      this.game.scale.gameSize.height > DESIGN_RES.height * this.dpr
    ) {
      const zoomX = this.game.scale.gameSize.width / (DESIGN_RES.width * this.dpr);
      const zoomY = this.game.scale.gameSize.height / (DESIGN_RES.height * this.dpr);
      return Math.min(zoomX, zoomY);
    }
    return this.calculateZoomFormula(DESIGN_RES.width, DESIGN_RES.height);
  }
}

export default CameraResizer;
